...
const ChatHeader = ({ userName, userImage, isAI = false, onBack, userId }: ChatHeaderProps) => {
  const ProfileContent = () => (
    <>
      <div className="relative">
        ...
        />
        {isAI && <span className="absolute -bottom-1 -right-1 w-3 h-3 bg-green-500 rounded-full border-2 border-dark-2" />}
      </div>
      <div>
        ...
        <span className="text-xs text-light-3">{isAI ? "Online" : "Active"}</span>
      </div>
    </>
  );
